...
const Hamburger = ({state}) => {
  ...
  useEffect(()=> {
    if(state.clicked === false) {
    ...
    } else if (state.clicked === true || state.clciked === true && state.initial === null) {
      //  open our menu
      gsap.to(menu, {
        duration: 0,
        css: {display: 'block'}
      ...
      staggerRevealBg(revealMenuBackground);
      staggerRevealMenu(revealMenu);
      fadeInUp(info);
      staggerText(line1, line2, line3);
    }
  }, [state]);

  const staggerRevealBg = (node1) => {
    gsap.from(node1, {
      duration: .8,
      height: 0,
          ...
          <div className="wrapper">
            <div className="menu-links">
              <nav>
                <ul>
                  <li>
                    <Link ref={el => {line1 = el}}  to="/opportunities">Opportunity</Link>
                  </li>
                  <li>
